...
export const getPublicConfigs = () => {
  return request.get<PublicConfig[]>('/public/websiteconfig')
}

// 根据Key获取公开配置
export const getPublicConfigByKey = (key: string) => {
  return request.get<PublicConfig>(`/public/websiteconfig/${key}`)
}

// 获取所有配置（管理员）
export const getWebsiteConfigs = (params?: WebsiteConfigQueryDto) => {
  return request.get<WebsiteConfigListResponse>('/admin/websiteconfig', { params })
}

// 获取配置分组列表
export const getConfigGroups = () => {
  return request.get<string[]>('/admin/websiteconfig/groups')
}

// 获取指定配置
export const getWebsiteConfig = (id: number) => {
  return request.get<WebsiteConfig>(`/admin/websiteconfig/${id}`)
}

// 创建配置
export const createWebsiteConfig = (data: CreateWebsiteConfigDto) => {
  return request.post<WebsiteConfig>('/admin/websiteconfig', data)
}

// 更新配置
export const updateWebsiteConfig = (id: number, data: UpdateWebsiteConfigDto) => {
  return request.put(`/admin/websiteconfig/${id}`, data)
}

// 批量更新配置
export const updateConfigsBatch = (configs: Record<string, string>) => {
  return request.put('/admin/websiteconfig/batch', configs)
}

// 删除配置
export const deleteWebsiteConfig = (id: number) => {
  return request.delete(`/admin/websiteconfig/${id}`)
}

// 初始化默认配置
export const initializeDefaultConfigs = () => {
  return request.post('/admin/websiteconfig/initialize')
}
