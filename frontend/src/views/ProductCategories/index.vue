    ...
    >
      ...
      >
        ...
        <el-form-item label="分类图片">
          <el-input v-model="categoryForm.image" placeholder="请输入图片URL" />
        </el-form-item>
...
import type { ProductCategory, CreateProductCategoryData, UpdateProductCategoryData } from '@/api/productCategories'
...
const filterCategories = (categories: ProductCategory[], excludeId: number): ProductCategory[] => {
...
const loadCategories = async () => {
  loading.value = true
  try {
    categories.value = await getProductCategoriesTree()
  } catch (error) {
...
const showCreateDialog = (parent?: ProductCategory) => {
...
const showEditDialog = (category: ProductCategory) => {
...
const saveCategory = async () => {
  ...
  try {
    ...
    if (editingCategory.value) {
      ...
      await updateProductCategory(editingCategory.value.id, updateData)
      ElMessage.success('更新产品分类成功')
    } else {
      ...
      ElMessage.success('创建产品分类成功')
    }

    dialogVisible.value = false
    loadCategories()
  } catch (error) {
    ElMessage.error('保存产品分类失败')
  } finally {
    saving.value = false
  }
}
